...
`optimal` generates large search tables at startup, so the smoke keeps its
solver checks small: it checks a solved cube after initialization and a
one-quarter-turn scramble. The binary is linked with a larger WASI stack because
Reid's table setup uses sizeable automatic arrays.

...
if [ "$#" -ne 8 ]; then
  echo "usage: test-wasi-sdk-standalone.sh BUILD_DIR DIST_DIR BIN_DIR GMP_DIR MPFR_DIR FLINT_DIR NTL_DIR PARI_DIR" >&2
  exit 2
fi
...
env \
  ...
  CPPFLAGS="-I$gmp_dir/include -I$mpfr_dir/include -I$flint_dir/include -I$ntl_dir/include -I$pari_dir/include" \
  CFLAGS="-Oz ${sjlj_flags[*]}" \
  CXXFLAGS="-Oz -std=c++17 ${sjlj_flags[*]}" \
  LDFLAGS="-L$gmp_dir/lib -L$mpfr_dir/lib -L$flint_dir/lib -L$ntl_dir/lib -L$pari_dir/lib ${sjlj_flags[*]} ${standalone_ldlibs[*]}" \
...
grep -F "FLINT_CFLAGS = -DFLINT=1 -I$flint_dir/include" libsrc/Makefile

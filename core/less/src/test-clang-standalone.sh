...
if [ "$#" -ne 7 ]; then
  echo "usage: test-clang-standalone.sh BUILD_DIR DIST_DIR BIN_DIR TERMCAP_DIST_DIR NCURSES_DIST_DIR POSIX_WASM_DIR SRC_DIR" >&2
  exit 2
fi
...
cowasm_clang_standalone_probe "less" "$bin_dir" "$probe_dir"
...
chmod -R u+w .
compat_dir="$build_dir/cowasm-clang-compat"
mkdir -p "$compat_dir"
...
#ifndef COWASM_CLANG_COMPAT_SETJMP_H
#define COWASM_CLANG_COMPAT_SETJMP_H
...
#ifndef COWASM_CLANG_COMPAT_SIGNAL_H
#define COWASM_CLANG_COMPAT_SIGNAL_H

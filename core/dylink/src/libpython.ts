...
const headers =
  "Python.h pyframe.h marshal.h frameobject.h structmember.h internal/pycore_unicodeobject.h internal/pycore_namespace.h internal/pycore_bytesobject.h internal/pycore_symtable.h internal/pycore_token.h internal/pycore_structseq.h internal/pycore_fileutils.h internal/pycore_runtime.h internal/pycore_capsule.h internal/pycore_code.h internal/pycore_pybuffer.h internal/pycore_object.h internal/pycore_emscripten_signal.h";

const includedHeaders = new Set(headers.split(/\s+/));
...
function cFunctionNameFromDeclaration(
  declaration: string
): string | undefined {
  const functionPointerReturn = declaration.match(
    /^\s*\*\s*\(\s*\*\s*([a-zA-Z_$][a-zA-Z\d_$]*)\s*\)\s*\(/
  );
  if (functionPointerReturn != null) {
    return functionPointerReturn[1];
  }
  const regularFunction = declaration.match(
    /^\s*\*?\s*([a-zA-Z_$][a-zA-Z\d_$]*)\s*\(/
  );
  return regularFunction?.[1];
}

function cDataNameFromDeclaration(declaration: string): string | undefined {
  const functionPointer = declaration.match(
    /^\s*\*\s*\(\s*\*\s*([a-zA-Z_$][a-zA-Z\d_$]*)\s*\)/
  );
  if (functionPointer != null) {
    return functionPointer[1];
  }
  const beforeSuffix = declaration.split(/[;(=]/)[0];
  const matches = Array.from(
    beforeSuffix.matchAll(/[a-zA-Z_$][a-zA-Z\d_$]*/g)
  );
...
export async function main(pathToPythonSource: string) {
  ...
  for (const grepLine of output.split("\n")) {
    const separator = grepLine.indexOf(":");
    if (separator == -1) {
      continue;
    }
    const filename = grepLine.slice(0, separator);
    const header = filename.startsWith(`${includeDir}/`)
      ? filename.slice(includeDir.length + 1)
      : filename;
    if (header.startsWith("internal/") && !includedHeaders.has(header)) {
      continue;
    }
    let line = grepLine.slice(separator + 1).trim();
    ...
    if (line.startsWith("PyAPI_FUNC")) {
      ...
      const name = cFunctionNameFromDeclaration(line);
      if (name == null || exclude.has(name)) continue;
      names.push(name);
      ...
      if (k == -1) continue;
      const name = cDataNameFromDeclaration(line.slice(k + 2));
      if (name == null || exclude.has(name)) continue;
      names.push(name);
    }
  }

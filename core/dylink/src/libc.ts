...
static int cowasm_ctype_upper[384];
static const unsigned short *cowasm_ctype_b_ptr;
static const int *cowasm_ctype_lower_ptr;
static const int *cowasm_ctype_upper_ptr;
static int cowasm_ctype_initialized = 0;

static void cowasm_init_ctype(void) {
  if (cowasm_ctype_initialized) return;
  for (int i = 0; i < 384; i++) {
    int c = i - 128;
    unsigned short flags = 0;
    if (c >= 9 && c <= 13) flags |= 0x2000;
    if (c == 32) flags |= 0x6000;
    if (c >= '0' && c <= '9') flags |= 0x0800;
    if (c >= 'A' && c <= 'Z') flags |= 0x0100;
    if (c >= 'a' && c <= 'z') flags |= 0x0200;
    cowasm_ctype_b[i] = flags;
    cowasm_ctype_lower[i] = c >= 'A' && c <= 'Z' ? c + 32 : c;
    cowasm_ctype_upper[i] = c >= 'a' && c <= 'z' ? c - 32 : c;
  }
  cowasm_ctype_b_ptr = cowasm_ctype_b + 128;
  cowasm_ctype_lower_ptr = cowasm_ctype_lower + 128;
  cowasm_ctype_upper_ptr = cowasm_ctype_upper + 128;
  cowasm_ctype_initialized = 1;
}

const unsigned short **__ctype_b_loc(void) {
  cowasm_init_ctype();
  return &cowasm_ctype_b_ptr;
}

const int **__ctype_tolower_loc(void) {
  cowasm_init_ctype();
  return &cowasm_ctype_lower_ptr;
}

const int **__ctype_toupper_loc(void) {
  cowasm_init_ctype();
  return &cowasm_ctype_upper_ptr;
}

double strtod_l(const char *nptr, char **endptr, struct __locale_struct *locale) {
  (void)locale;
  return strtod(nptr, endptr);
}

float strtof_l(const char *nptr, char **endptr, struct __locale_struct *locale) {
  (void)locale;
  return strtof(nptr, endptr);
}

long strtol_l(const char *nptr, char **endptr, int base, struct __locale_struct *locale) {
  (void)locale;
  return strtol(nptr, endptr, base);
}

long long strtoll_l(const char *nptr, char **endptr, int base, struct __locale_struct *locale) {
  (void)locale;
  return strtoll(nptr, endptr, base);
}

unsigned long strtoul_l(const char *nptr, char **endptr, int base, struct __locale_struct *locale) {
  (void)locale;
  return strtoul(nptr, endptr, base);
}

unsigned long long strtoull_l(const char *nptr, char **endptr, int base, struct __locale_struct *locale) {
  (void)locale;
  return strtoull(nptr, endptr, base);
}

void *__cxa_begin_catch(void *ptr) { return ptr; }
void *__cxa_find_matching_catch_2(void) { return 0; }
void *__cxa_find_matching_catch_3(void *ptr) { return ptr; }
void *__cxa_current_primary_exception(void) { return 0; }
int __cxa_uncaught_exceptions(void) { return 0; }
void __cxa_end_catch(void) {}
void __cxa_rethrow(void) { abort(); }
void __cxa_rethrow_primary_exception(void *ptr) { abort(); }
void __cxa_throw(void *thrown_exception, void *tinfo, void (*dest)(void *)) { abort(); }
void __resumeException(void *ptr) { abort(); }
int getTempRet0(void) { return 0; }
int __THREW__ = 0;
int __threwValue = 0;
...
export function libcExportCode() {
  let s = `
...
const wasiSdkDataSymbols = new Set([
  "_CLOCK_MONOTONIC",
...
function allLibcExportNames(): string[] {
  const excluded = new Set(["__SIG_ERR", "signal"]);
  return Array.from(
    new Set(
      (symbols + "\n" + posix + "\n" + cxxRuntimeHelperSymbols)
        .split("\n")
        .map((name) => name.trim())
...
export function libcWasiSdkExportCode() {
  const names = allLibcExportNames();
  const declarations = names
    .map((name) =>
      wasiSdkDataSymbols.has(name)
        ? `extern char ${name};`
...
/*
 * This export source is intentionally declaration-only. It is compiled with
 * wasi-sdk for the dylink runtime archive, whose job is to make main-module
 * libc symbols discoverable without depending on Zig/musl-only headers.
 */
...
if (require.main === module) {
  console.log(
    process.argv.includes("--wasi-sdk")
      ? libcWasiSdkExportCode()
      : libcExportCode()
  );
}

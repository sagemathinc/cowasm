...
if [ "$#" -ne 5 ]; then
  echo "usage: test-wasi-sdk-standalone.sh BUILD_DIR DIST_DIR BIN_DIR NATIVE_F2C SRC_DIR" >&2
  exit 2
fi
...
cowasm_standalone_probe "f2c" wasi-sdk "$bin_dir" "$probe_dir"
...
env COWASM_TOOLCHAIN=wasi-sdk "$bin_dir/cowasm-cc" \
  ...
  -I"$dist_dir/include" -L"$dist_dir/lib" -lf2c "$probe_dir/compat.o"

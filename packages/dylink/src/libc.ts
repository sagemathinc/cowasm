...
const headers =
  "assert.h ctype.h dirent.h errno.h langinfo.h locale.h math.h nl_types.h regex.h setjmp.h signal.h stdarg.h stdio.h stdlib.h string.h strings.h time.h wchar.h wctype.h sys/select.h unistd.h sys/stat.h malloc.h termios.h fcntl.h";

const functions =
  "abort abs acos asctime asctime_r asin assert atan atan2 atexit atof atoi atol bsearch btowc calloc catclose catgets catopen ceil clearerr clock confstr cos cosh ctime ctime64 ctime64_r ctime_r difftime difftime64 div erf erfc exit exp fabs fclose fdopen feof ferror fflush fgetc fgetpos fgets fgetwc fgetws fileno floor fmod fopen fprintf fputc fputs fputwc fputws fread free freopen frexp fscanf fseek fsetpos ftell fwide fwprintf fwrite fwscanf gamma getc getchar getcwd getdelim getenv getline gets getwc getwchar gmtime gmtime64 gmtime64_r gmtime_r hypot isalnum isalpha isascii isatty isblank iscntrl isdigit isgraph islower isprint ispunct isspace isupper iswalnum iswalpha iswblank iswcntrl iswctype iswdigit iswgraph iswlower iswprint iswpunct iswspace iswupper iswxdigit isxdigit j0 j1 jn labs ldexp ldiv localeconv localtime localtime64 localtime64_r localtime_r log log10 longjmp lseek  mblen mbrlen mbrtowc mbsinit mbsrtowcs mbstowcs mbtowc memchr memcmp memcpy memmove memset mktime mktime64 modf nextafter nextafterl nexttoward nexttowardl nl_langinfo opendir perror pow pread printf putc putchar putenv puts putwc putwchar quantexpd128 quantexpd32 quantexpd64 quantized128 quantized32 quantized64 raise rand rand_r read readdir readlink realloc regcomp regerror regexec regfree remove rename rewind samequantumd128 samequantumd32 samequantumd64 scanf setbuf setjmp setlocale setvbuf signal sin sinh sprintf snprintf sprintf sqrt srand sscanf stat strcasecmp strcat strchr strcmp strcoll strcpy strcspn strdup strerror strfmon strftime strlen strncasecmp strncat strncmp strncpy strndup strpbrk strptime strrchr strspn strstr strtod strtod128 strtod32 strtod64 strtof strtok strtok_r strtol strtold strtoul strxfrm swprintf swscanf system tan tanh time time64 tmpfile tmpnam toascii tolower toupper towctrans towlower towupper ungetc ungetwc unsetenv va_arg va_copy va_end va_start vfprintf vfscanf vfwprintf vfwscanf vprintf vscanf vsnprintf vsprintf vsscanf vswprintf vswscanf vwprintf vwscanf wcrtomb wcscat wcschr wcscmp wcscoll wcscpy wcscspn wcsftime wcslen wcslocaleconv wcsncat wcsncmp wcsncpy wcspbrk wcsptime wcsrchr wcsrtombs wcsspn wcsstr wcstod wcstod128 wcstod32 wcstod64 wcstof wcstok wcstol wcstold wcstombs wcstoul wcsxfrm wctob wctomb wctrans wctype wcwidth wmemchr wmemcmp wmemcpy wmemmove wmemset wprintf wscanf y0 y1 yn select pselect FD_CLR FD_ISSET FD_SET FD_ZERO  malloc malloc_usable_size tcsetattr open write sysconf fstat closedir fcntl __wasilibc_tell close ";

const symbols =
  "stdin stdout stderr errno _CLOCK_MONOTONIC _CLOCK_REALTIME _CLOCK_PROCESS_CPUTIME_ID _CLOCK_THREAD_CPUTIME_ID";
...
const aliases = { siprintf: "sprintf", iprintf: "printf" };

...
//     try node.registerFunction(env, exports, "getresgid", getresgid);
//     try node.registerFunction(env, exports, "setresuid", setresuid);
//     try node.registerFunction(env, exports, "setresgid", setresgid);
}

// These uid_t and gid_t are actually u32.
extern fn getresuid(uid: *unistd.uid_t, euid: *unistd.uid_t, suid: *unistd.uid_t) c_int;
...
// fn getresgid(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
//     _ = info;
//     var rgid: unistd.gid_t = undefined;
//     var egid: unistd.gid_t = undefined;
//     var sgid: unistd.gid_t = undefined;
//     if (unistd.getresgid(&rgid, &egid, &sgid) == -1) {
...
// fn setresgid(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
...
//     if (unistd.setresgid(rgid, egid, sgid) == -1) {
...
// fn setresuid(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
//     const argv = node.getArgv(env, info, 3) catch return null;
...
//     if (unistd.setresuid(ruid, euid, suid) == -1) {

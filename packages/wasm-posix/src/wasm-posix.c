...
ssize_t getrandom(void* buf, size_t buflen, unsigned int flags) {
  STUB(
      "getrandom: TODO -- definitely need to actually randomize the buffer at "
      "some point!")
  return buflen;
}
...
int sigaction(int signum, const struct sigaction* act,
              struct sigaction* oldact){STUB("sigaction")}

pid_t fork(void){STUB("fork")}

pid_t fork1(void){STUB("fork1")}

ssize_t splice(int fd_in, void* off_in, int fd_out, void* off_out, size_t len,
               unsigned int flags) {
  STUB("splice")
}
int login_tty(int fd) { STUB("login_tty") }
int eventfd(unsigned int initval, int flags) { STUB("eventfd"); }
...
int sigwait(const sigset_t* set, int* sig) { STUB("sigwait") }
int sigfillset(sigset_t* set) { STUB("sigfillset") }

int sigwaitinfo(const sigset_t* set, siginfo_t* info) { STUB("sigwaitinfo") }
...
int sigaltstack(const stack_t* ss, stack_t* old_ss) { STUB("sigaltstack") }
...
void* dlsym(void* handle, const char* symbol) { STUB("dlsym") }
...
char* realpath(const char* path, char* resolved_path) { STUB("realpath") }

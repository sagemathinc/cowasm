...
smoke path, including the staged smoke script itself, so incomplete resource
copies fail before the worker starts.
...
requirements.
The manifest also records required native library resources such as
`libcxx.so`, so packaging validation catches missing side-module companion
libraries before the worker starts.
